...
const initializeGOLibraryWebComponents_DEVELOPMENT = () => {
  import('@geneontology/wc-ribbon-strips/loader')
    .then(({applyPolyfills, defineCustomElements}) =>
      applyPolyfills().then(_ => defineCustomElements(window))
    );

  import('@geneontology/wc-ribbon-table/loader')
    .then(({applyPolyfills, defineCustomElements}) =>
      applyPolyfills().then(_ => defineCustomElements(window))
    );

  import('@geneontology/wc-gocam-viz/loader')
    .then(({applyPolyfills, defineCustomElements}) =>
      applyPolyfills().then(_ => defineCustomElements(window))
    );
}
