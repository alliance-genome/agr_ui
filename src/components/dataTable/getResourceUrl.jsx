...
export const getResourceUrl = ({ identifier, type, subtype }) => {
  ...
  if (subtype) {
    [resource] =
      resourceDescriptors
        .find((resource) => resource.db_prefix === prefix)
        ?.pages.find((page) => page.name === subtype.name.toLowerCase())
        ?.url.split('[') || [];
  } else if (type) {
    [resource] =
      resourceDescriptors
        .find((resource) => resource.db_prefix === prefix)
        ?.pages.find((page) => page.name === type.toLowerCase())
        ?.url.split('[') || [];
  } else {
    if (prefix === 'ORPHA') {
      prefix = 'Orphanet';
    }
    [resource] = resourceDescriptors.find((resource) => resource.db_prefix === prefix)?.default_url.split('[') || [];
  }
  return resource + id;
};
