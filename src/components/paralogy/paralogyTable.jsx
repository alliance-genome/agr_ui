          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>
                  Rank{' '}
                  <HelpPopup id={`help-paralogy-rank`}>
                    <RankHelp />
                  </HelpPopup>
                </th>
                <th>
                  Alignment Length (aa){' '}
                  <HelpPopup id={`help-paralogy-align`}>
                    <AlignmentHelp />
                  </HelpPopup>
                </th>
                <th>Similarity %</th>
            ...
            <tbody>
              {data.results?.map((result) => {
                const rowKey =
                  'paralogyrowkey-' + result.geneToGeneParalogy.objectGene.primaryExternalId.replace(/\s/g, '-');
                return (
                  <tr key={rowKey}>
                    <td>
                      <Link to={`/gene/${result.geneToGeneParalogy.objectGene.primaryExternalId}`} target="_blank">
                        <span
                          dangerouslySetInnerHTML={{
                            __html: result.geneToGeneParalogy.objectGene.geneSymbol.displayText,
                          }}
                        />
                      </Link>
                    </td>
                    ...
                    <td>
                      {result.geneToGeneParalogy.predictionMethodsMatched.length} of {getTotalMethodCount(result)}
                    </td>
                    <MethodCell
