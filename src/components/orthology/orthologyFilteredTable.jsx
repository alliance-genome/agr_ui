...
import { compareAlphabeticalCaseInsensitive, orthologyMeetsStringency } from '../../lib/utils';
...
const OrthologyFilteredTable = ({ geneId }) => {
  const [filterScoreGreaterThan, setFilterScoreGreaterThan, resetFilterScoreGreaterThan] = useResettableState(0);
  const [filterMethod, setFilterMethod, resetFilterMethod] = useResettableState(null);
  const [filterBest, setFilterBest, resetFilterBest] = useResettableState(false);
  const [filterReverseBest, setFilterReverseBest, resetFilterReverseBest] = useResettableState(false);
  const [filterSpecies, setFilterSpecies, resetFilterSpecies] = useResettableState(null);
  ...
  const filterCallback = (dat) => {
    const meetMethodFilter = filterMethod
      ? dat.predictionMethodsMatched.some((method) => method.name === filterMethod)
      : true;
  ...
  const all_methods = data.results[0].predictionMethodsMatched
    .concat(data.results[0].predictionMethodsNotCalled, data.results[0].predictionMethodsNotMatched)
  ...
  return (
    <div>
      <ControlsContainer>
        <StringencySelection level={stringencyLevel} onChange={setStringencyLevel} />
            ...
            <label style={labelStyle}>
              ...
              >
                ...
                {all_methods.map((method, index) => {
                  ...
                  return (
                    <option key={scoreGreaterThanValue} value={scoreGreaterThanValue}>
                      &gt; {scoreGreaterThanValue}
                    </option>
                  );
                })}
              </select>
        ...
        <div>
          ...
          <button className="btn btn-outline-secondary" onClick={resetFilters} style={buttonStyle} type="button">
            Reset filters
          </button>
        </div>
      </ControlsContainer>
