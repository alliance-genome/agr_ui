...
import { STRINGENCY_HIGH, STRINGENCY_MED, STRINGNECY_LOW } from './homology/constants';
...
import { getOrthologId, getOrthologSpeciesId, getOrthologSymbol } from './orthology';
...
const bySpecies = (species) => (orthology) =>
  species.map((s) => s.taxonId).indexOf(getOrthologSpeciesId(orthology)) >= 0;
...
const sortBySpecies = (species) =>
  species.sort(
    compareByFixedOrder(
      SPECIES.map((s) => s.taxonId),
      (s) => s.taxonId
    )
  );

const uniqueSpecies = (species) =>
  species.filter((element, idx, array) => array.map((a) => a.taxonId).indexOf(element.taxonId) === idx);
...
const OrthologPicker = ({
  ...
  useEffect(() => {
    ...
    if (checkboxValue && orthologyResults) {
      selectedOrthologs = orthologyResults
  ...
  const toggleVertebrates = (checked) => {
    ...
    if (checked) {
      setSelectedSpecies((current) =>
        uniqueSpecies(sortBySpecies([...current, ...SPECIES.filter((s) => s.vertebrate)]))
      );
    } else {
  ...
  const speciesHasOrthologsWithData = (species) => {
    ...
    return orthologyResults?.filter(bySpecies([species])).some((o) => geneHasData(o, getOrthologId(o)));
  };

  const speciesHasOrthologsMeetingStringency = (species) => {
    return orthologyResults
      ...
      <div className="ml-3">
        <UncontrolledDropdown className="pr-2" tag="span">
          ...
          <DropdownMenu>
            <form className="px-4 py-3" style={{ minWidth: '300px' }}>
              ...
              <button className="btn btn-outline-secondary" onClick={() => setStringency(null)} type="button">
                Clear
              </button>
            </form>
          </DropdownMenu>
        ...
        <UncontrolledDropdown className="pr-2" tag="span">
          <DropdownToggle
            ...
            color="primary"
            disabled={inputDisabled}
            outline={!checkboxValue || !selectedSpecies.length}
          >
            Species
            {selectedSpecies.length > 0 && (
              <span>
                : <SpeciesName>{selectedSpecies[0].fullName}</SpeciesName>
              </span>
            )}
          ...
          <DropdownMenu>
            <form className="px-4 py-3" style={{ minWidth: '300px' }}>
              ...
              <hr />
              <div className="form-group">
                {SPECIES.filter((species) => (focusTaxonId ? species.taxonId !== focusTaxonId : true)).map(
                  (species) => {
                    const checkId = id + makeId(species.taxonId);
                    const hasOrthologs =
                      orthologyResults?.findIndex((o) => getOrthologSpeciesId(o) === species.taxonId) >= 0;
                      ...
                      <div key={species.taxonId}>
                        ...
                        {disabled && (
                          <UncontrolledTooltip delay={{ show: 200, hide: 0 }} placement="bottom" target={checkId}>
                            {(!hasOrthologs && (
                              <>
                                No <SpeciesName>{species.fullName}</SpeciesName> orthologs of this gene
                              </>
                            )) ||
                              (!hasOrthologsWithData && (
                                <>
                                  No <SpeciesName>{species.fullName}</SpeciesName> orthologs of this gene have
                                  annotations
                                </>
                              )) ||
                              (!hasOrthologsMeetingStringency && (
                                <>
                                  No <SpeciesName>{species.fullName}</SpeciesName> orthologs of this gene meet selected
                                  Stringency filter
                                </>
                              ))}
                          </UncontrolledTooltip>
              ...
              <button className="btn btn-outline-secondary" onClick={handleClearSpecies} type="button">
                Clear
              </button>
            </form>
          </DropdownMenu>
