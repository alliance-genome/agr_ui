...
describe('Yeast Footer', () => {
  ...
  it('Should render hex with link to mod main page, name ', () => {
    ...
    expect(top_link).toHaveAttribute('href', 'https://www.yeastgenome.org');

    expect(top_div).toContainElement(top_label);
    expect(top_label).toHaveTextContent('SGD');

    expect(top_link).toContainElement(link_img);
    expect(link_img).toHaveAttribute('src', 'alliance_logo_sgd.png');
  });
  ...
  if (
    ('Should render about SGD as the first footer link',
    () => {
