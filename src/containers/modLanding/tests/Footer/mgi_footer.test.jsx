...
describe('Mouse footer tests.', () => {
  ...
  it('Should render hex with link to mod main page, name ', () => {
    ...
    expect(top_link).toHaveAttribute('href', 'http://www.informatics.jax.org/');

    expect(top_div).toContainElement(top_label);
    expect(top_label).toHaveTextContent('MGD');

    expect(top_link).toContainElement(link_img);
    expect(link_img).toHaveAttribute('src', 'alliance_logo_mgd.png');
  });
  ...
  if (
    ('Should render citing Citing MGI Resources as the first footer link',
    () => {
      ...
      expect(label).toContainHTML('Citing MGI Resources');
    })
  );
});
