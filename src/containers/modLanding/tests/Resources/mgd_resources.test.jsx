...
describe('MGD Resources', () => {
  ...
  if (
    ('Should render Mouse Genes as the first footer link',
    () => {
      const footer = screen(getByTestId('href_resources_0')); // First item
      expect(footer).toHaveAttribute('href', 'http://www.informatics.jax.org/genes.shtml');
      const label = screen.getByTestId('resources_label_0');
      expect(label).toContainHTML('Mouse Genes');
    })
  );
});
