...
describe('ZFIN Search', () => {
  ...
  if (
    ('Should render D. rerio search as the first search link',
    () => {
      const footer = screen(getByTestId('href_search_0')); // First item
      expect(footer).toHaveAttribute('href', 'search?species=Danio rerio&category=gene_search_result');
