...
describe('SGD Search', () => {
  ...
  if (
    ('Should render S. cerevisiae search as the first search link',
    () => {
      const footer = screen(getByTestId('href_search_0')); // First item
      expect(footer).toHaveAttribute('href', 'search?species=Saccharomyces cerevisiae&category=gene_search_result');
