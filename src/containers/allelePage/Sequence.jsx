...
const Sequence = ({ sequence = '', maxCharacterLengthInline = 20, renderLink }) => {
  ...
  return sequence.length > maxCharacterLengthInline ? (
    <>
      ...
      <Modal fade={false} isOpen={modal} toggle={toggle}>
        ...
        <ModalFooter>
          <button className="btn btn-outline-secondary" onClick={toggle} type="button">
            Cancel
          </button>
        </ModalFooter>
      </Modal>
    ...
    <span className={`${style.sequence} ${style.sequenceInline}`}>{sequence}</span>
  );
};
