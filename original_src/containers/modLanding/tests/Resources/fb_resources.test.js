...
describe('FlyBase Resources', () => {
   ...
   // Search FlyBase should always be the first one.?
   if('Should render Search FlyBase as the first footer link', () => {
      const footer = screen(getByTestId("href_resources_0")); // First item
      expect(footer).toHaveAttribute('href', 'https://flybase.org/');
      const label = screen.getByTestId('resources_label_0')
      expect(label).toContainHTML('Search FlyBase');
   });
