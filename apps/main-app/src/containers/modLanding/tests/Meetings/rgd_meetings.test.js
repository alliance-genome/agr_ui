...
import '@testing-library/jest-dom'
 ...
 describe('App', () => {
     beforeEach(() => {
       render(<Provider store={store}> <MeetingsRgd
              urlMeetingsMod={content.rgdMeetingsAPI}
              linkToMeetingsPage={content.linkToMeetingsPage}
              fetchMeetingsCount={content.fetchMeetingsCount}/></Provider>)
     });

     it('should render successfully', async () => {
       const meetings_div = await waitFor(() => screen.findByTestId("div_meetings_2"), { timeout: 8000 });
       expect(screen.getByTestId("div_meetings_2")).toHaveTextContent("Update on rat title 1");
     }, 10000);
     ...
     it('should contain a link', async () => {
      ...
      it('meetings href should have a header and be equal to', async () => {
         const meetings_div = await waitFor(() => screen.findByTestId("div_meetings_2"), { timeout: 8000 });
         const head = screen.getByTestId("header_meetings_2");
         expect(head).toHaveTextContent("Update on rat title 1");
      }, 10000);
