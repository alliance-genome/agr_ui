...
const ParalogyFilteredTable = ({geneId}) => {
  const [
      ...
      <div style={{marginBottom: '1rem'}}>
        {
          filteredData.length > 0 ?
            <HorizontalScroll width={800}>
              <ParalogyTable data={filteredData} />
            </HorizontalScroll> :
            <NoData>No paralog matching your filter. Please try a less stringent filter.</NoData>
        }
      </div>
    </div>
  );
...
ParalogyFilteredTable.propTypes = {
  geneId: PropTypes.string.isRequired,
};

export default ParalogyFilteredTable;
